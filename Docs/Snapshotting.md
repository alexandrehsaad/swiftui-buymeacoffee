...
`Scripts/GenerateTestHost.swift` creates the temporary project under `.build/tests/Host`. Its `TestHost`
...
After generation, you can also open `TestHost.xcodeproj` in Xcode, select the `TestHost` scheme and matching
...
The `Run Snapshot Tests (iOS)` job in the `Tests` workflow compares existing references using Xcode 26.5 and an iPhone
