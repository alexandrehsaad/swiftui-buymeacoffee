...
fileprivate struct Arguments {
    ...
    /// - Throws: `CodeCoverageReportError.invalidArguments` if the arguments do not contain one report and one source
    ///   directory.
...
fileprivate struct CodeCoverageReporter {
    ...
    /// - Throws: `CodeCoverageReportError` if the report cannot be read, decoded, or does not contain package source
    ///   coverage.
    fileprivate init(
        reportURL: URL,
        sourcesURL: URL
